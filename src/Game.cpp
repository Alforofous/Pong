...
bool Game::initialise(sf::Vector2f pitchSize)
{
    ...
    while (index.y < BLOCK_HEIGHT)
    {
        ...
        {
            if (index.y >= 0 && index.y < BLOCK_HEIGHT / 4)
                block[index.y][index.x] = 1;
            else if (index.y >= BLOCK_HEIGHT / 4 * 3 && index.y <= BLOCK_HEIGHT)
                block[index.y][index.x] = 1;
            else
                block[index.y][index.x] = 0;
            index.x += 1;
        }
        index.y += 1;
    }
...
void Game::draw(sf::RenderTarget &target, sf::RenderStates states) const
{
    ...
    m_pPaddles[Side::LEFT].get()->side = 0;
    target.draw(*m_pPaddles[Side::LEFT].get());
    m_pPaddles[Side::RIGHT].get()->side = 1;
    ...
    leftScore.setPosition(target.getSize().x * 0.25f, target.getSize().y * 0.47f);
    leftScore.setStyle(sf::Text::Bold);
    target.draw(leftScore);
    
    leftScore.setString(std::to_string(m_score[Side::RIGHT]));
    leftScore.setPosition(target.getSize().x * 0.75f, target.getSize().y * 0.47f);
    target.draw(leftScore);
}
